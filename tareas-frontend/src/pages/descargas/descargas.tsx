...
import { reportesService } from "../../main";
...
export default function Descargas() {
  ...
  const obtenerReportes = async () => {
    try {
      const reportes = await reportesService.miListado(
        debouncedBusqueda,
        pagina,
        limite,
        filtros.estado
      );
      setReportes(reportes.data as Reportes[]);
      setTotal(reportes.total as number);
    } catch (error: any) {
            ...
            {reportes.length > 0 ? (
              reportes.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell className="text-body">{r.error || "-"}</TableCell>
