...
export class RecordatorioTareasCronService {
  ...
  async notificarUsuariosInactivos() {
    ...
    const usuarios = await this.usuariosService.find({
      where: { estatus: true },
    });

    for (const usuario of usuarios) {
      const tareas = await this.tareasService.find({
      ...
      const tokens = await this.tokensFCMService.find({
        where: { id_usuario: usuario.id },
      });
      ...
      for (const token of tokens) {
        try {
          ...
          if (
            ...
            await this.tokensFCMService.deleteWhere({
              where: { id: token.id },
            });
  ...
  @Cron(CronExpression.EVERY_5_MINUTES) async notificarTareasProximas() {
    this.logger.log('Ejecutando notificación de tareas próximas a vencer...');
    const tareas = await this.tareasService.obtenerTareasProximas(24);
    if (tareas.length === 0) {
      this.logger.log('No hay tareas próximas a vencer.');
      return;
    }
    const tareasPorUsuario = new Map<number, any[]>();
    for (const tarea of tareas) {
      if (!tareasPorUsuario.has(tarea.id_usuario)) {
        tareasPorUsuario.set(tarea.id_usuario, []);
      }
      tareasPorUsuario.get(tarea.id_usuario).push(tarea);
    }
    for (const [idUsuario, tareasUsuario] of tareasPorUsuario.entries()) {
      const tokens = await this.tokensFCMService.find({
        where: { id_usuario: idUsuario },
      });
      if (!tokens || tokens.length === 0) {
        this.logger.warn(`Usuario ${idUsuario} no tiene tokens registrados`);
        continue;
      }
      for (const token of tokens) {
        try {
          await this.firebaseService.sendPushNotification(
            ...
            `Tienes ${tareasUsuario.length} tarea(s) próxima(s) a vencer.`,
          );
          this.logger.log(`Notificación enviada a usuario ${idUsuario}`);
        } catch (err) {
          if (
            err.code === 'messaging/registration-token-not-registered' ||
            err.code === 'messaging/invalid-argument'
